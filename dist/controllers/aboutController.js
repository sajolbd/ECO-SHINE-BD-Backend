...
const getAbout = async (req, res, next) => {
    try {
        let about = await About_1.About.findOne({});
        if (!about) {
            about = await About_1.About.create(DEFAULT_ABOUT);
        }
...
const updateAbout = async (req, res, next) => {
    try {
        let about = await About_1.About.findOne({});
        if (!about) {
            about = await About_1.About.create({ ...DEFAULT_ABOUT, ...req.body });
        }
        else {
            about = await About_1.About.findByIdAndUpdate(about._id, req.body, {
                new: true,
                runValidators: true,
            });
        }
