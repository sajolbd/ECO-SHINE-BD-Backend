...
exports.updateSEO = exports.getSEO = void 0;
...
const getSEO = async (req, res, next) => {
    try {
        ...
        let seo = await SEO_1.SEO.findOne({ pageName: pageName.toLowerCase() });
        if (!seo) {
            const defaultPageData = DEFAULT_SEO_METADATA[pageName.toLowerCase()];
            if (defaultPageData) {
                seo = await SEO_1.SEO.create(defaultPageData);
            }
            else {
                seo = await SEO_1.SEO.create({
...
const updateSEO = async (req, res, next) => {
    try {
        ...
        if (!seo) {
            seo = await SEO_1.SEO.create({ ...defaultPageData, ...req.body });
        }
        else {
            seo = await SEO_1.SEO.findOneAndUpdate({ pageName: pageName.toLowerCase() }, req.body, {
                new: true,
                runValidators: true,
            });
        }
