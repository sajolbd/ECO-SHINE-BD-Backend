...
const getAdminUsers = async (req, res, next) => {
    try {
        const users = await AdminUser_1.AdminUser.find({}).select("-passwordHash").sort({ createdAt: -1 });
...
const createAdminUser = async (req, res, next) => {
    try {
        ...
        const exists = await AdminUser_1.AdminUser.findOne({ email: email.toLowerCase() });
        if (exists) {
            return res.status(400).json({ success: false, message: "An admin user with this email already exists." });
        }
        const salt = await bcryptjs_1.default.genSalt(10);
        const passwordHash = await bcryptjs_1.default.hash(password, salt);
        const user = await AdminUser_1.AdminUser.create({
            name,
...
const updateAdminUser = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, email, password, role } = req.body;
        const user = await AdminUser_1.AdminUser.findById(id);
        if (!user) {
...
const deleteAdminUser = async (req, res, next) => {
    try {
        ...
        if (user.role === "super-admin") {
            const superAdminCount = await AdminUser_1.AdminUser.countDocuments({ role: "super-admin" });
            if (superAdminCount <= 1) {
                return res.status(400).json({ success: false, message: "You cannot delete the last Super Admin account." });
            }
        }
        await AdminUser_1.AdminUser.findByIdAndDelete(id);
