...
const updateHomepage = async (req, res, next) => {
    try {
        ...
        if (!updateData.heroHeading || !updateData.heroHeading.trim())
            updateData.heroHeading = DEFAULT_HOMEPAGE.heroHeading;
        if (!updateData.heroDescription || !updateData.heroDescription.trim())
            updateData.heroDescription = DEFAULT_HOMEPAGE.heroDescription;
        if (!updateData.heroDesktopImage || !updateData.heroDesktopImage.trim())
            updateData.heroDesktopImage = DEFAULT_HOMEPAGE.heroDesktopImage;
        if (!updateData.heroMobileImage || !updateData.heroMobileImage.trim())
            updateData.heroMobileImage = DEFAULT_HOMEPAGE.heroMobileImage;
        if (!updateData.announcements || updateData.announcements.length === 0)
            updateData.announcements = DEFAULT_HOMEPAGE.announcements;
        if (!homepage) {
            homepage = await Homepage_1.Homepage.create({ ...DEFAULT_HOMEPAGE, ...updateData });
        }
        else {
            homepage = await Homepage_1.Homepage.findByIdAndUpdate(homepage._id, updateData, {
                new: true,
                runValidators: true,
            });
        }
